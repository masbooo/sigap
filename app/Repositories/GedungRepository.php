...
class GedungRepository extends LegacyRepository
{
    ...
    {
        $rows = $this->getActiveBuildingRows(true);
    ...
    {
        $rows = $this->getActiveBuildingRows();
    ...
    private function getActiveBuildingRows(bool $regionOrdered = false): array
    {
        $perHourPriceSql = $this->getAliasedColumnSql('gedung', 'g', 'perhour_price', 'overtime_price');
        $query = $this->activeBuildingQuery()
            ->select([
                'g.id',
                'g.building_name',
                'g.description',
        ...
        if ($regionOrdered) {
            $query->orderByRaw("FIELD(k.region, 'Pusat', 'Timur', 'Selatan', 'Barat', 'Utara')")
                ->orderBy('k.district');
        }

        return $this->rows($query->orderBy('g.building_name')->get());
    }

    private function activeBuildingQuery()
    {
        $thumbnailQuery = $this->table('foto_gedung')
            ->select('building_id')
        ...
        return $this->query()
            ->from('gedung as g')
            ->leftJoinSub($thumbnailQuery, 'fg', 'fg.building_id', '=', 'g.id')
