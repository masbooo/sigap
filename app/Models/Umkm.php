...
class Umkm
{
    ...
    protected function getActiveLinkedItems(): array
    {
        $sql = "
            ...
            FROM umkm u
            INNER JOIN gedung_umkm gu
                ON gu.umkm_id = u.id
            INNER JOIN gedung g
                ON g.id = gu.gedung_id
               AND g.status = 'AKTIF'
            LEFT JOIN produk p
                ON p.id = u.product_id
               AND p.status = 'AKTIF'
            LEFT JOIN kecamatan kd
                ON kd.id = u.district_id
            LEFT JOIN kelurahan kl
                ON kl.id = u.subdistrict_id
            LEFT JOIN kecamatan kg
                ON kg.id = g.district_id
    ...
    protected function getActiveReservationRows(?int $umkmId = null): array
    {
        ...
        if ($umkmId !== null && $umkmId > 0) {
            $sql .= " AND u.id = :umkm_id";
            $params[':umkm_id'] = $umkmId;
        }

        $sql .= " ORDER BY u.umkm_name ASC, g.building_name ASC";
